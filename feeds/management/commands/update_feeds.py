...
class Command(BaseCommand):
    ...
    def save_storify_story(self, story):
        ...
        s = Story.objects.create(key=story['sid'],
                                 title=story['title'],
                                 published=pubdate,
    ...
    def check_storify_for_updates(self, story, existing_story):
        """Checks a to see if a Storify story we already know about has been updated."""
    ...
    def fetch_from_storify(self):
        ...
        for story in stories:
            try:
                ...
                self.check_storify_for_updates(story, existing_story)
    ...
    def save_tweetminster_story(self, story):
        """Save a Tweetminster story."""
        try:
            thumbnail =  story['embed'][0]['thumbnail_url']
        except KeyError:
            thumbnail = ''

        s = Story.objects.create(key=story['_id'],
                                 title=story['title'],
                                 description=story['description'],
                                 source='Tweetminster',
                                 permalink=story['uri'],
    ...
    def fetch_from_content_api(self):
        ...
        content_api_key = settings.CONTENT_API_KEY
        r = requests.get('http://content.guardianapis.com/search?tag=sport&page-size=50&format=json&show-fields=trailText&show-media=picture&api-key=%s' % content_api_key)
    ...
    def find_ten_with_images(self, stories):
        """Finds articles in the Content API with images."""
        withimages = []
        for story in stories:
    ...
    def save_content_api_story(self, story):
        ...
        s = Story.objects.create(key=story['id'],
                                 title=story['webTitle'],
                                 description=story['fields']['trailText'],
                                 source='ContentAPI',
                                 permalink=story['webUrl'],
                                 latest=True,
                                 thumbnail=bigpicture['file']).save()
